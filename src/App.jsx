...
const Inp = ({err,style={},booth=false,...p}) => <input {...p} style={{width:"100%",padding:"9px 12px",fontSize:13,border:`1.5px solid ${err?"#EF4444":booth?C.teal+"66":C.gray200}`,borderRadius:9,background:C.white,color:C.gray900,outline:"none",transition:"border .15s",fontFamily:"inherit",...style}}/>;
const Sel = ({err,children,booth=false,...p}) => <select {...p} style={{width:"100%",padding:"9px 12px",fontSize:13,border:`1.5px solid ${err?"#EF4444":booth?C.teal+"66":C.gray200}`,borderRadius:9,background:C.white,color:C.gray900,outline:"none",fontFamily:"inherit"}}>{children}</select>;
...
function Btn({children,onClick,v="default",disabled=false,style={},title=""}) {
  ...
  return <button title={title} onClick={onClick} disabled={disabled} style={{display:"inline-flex",alignItems:"center",justifyContent:"center",gap:6,padding:"9px 18px",fontSize:13,fontWeight:700,borderRadius:10,cursor:disabled?"not-allowed":"pointer",transition:"all .15s",fontFamily:"inherit",opacity:disabled?.6:1,whiteSpace:"nowrap",...styles[v],...style}}>{children}</button>;
}
...
function ErrorScreen({message,onRetry}) {
  return (
    <div style={{display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",height:"100vh",gap:16,background:C.white,padding:40,textAlign:"center"}}>
      <div style={{fontSize:40}}>❌</div>
      <div style={{fontSize:16,fontWeight:800,color:C.red}}>Failed to load</div>
...
// ─── OTP MODAL (for admin actions inside app) ─────────────────────────────────
function OTPModal({open,onClose,onSuccess,pin}) {
  const [v,setV]=useState(""); const [err,setErr]=useState("");
  const go=()=>{if(v===pin){setV("");setErr("");onSuccess();}else setErr("Incorrect PIN.");};
  return (
    <Modal open={open} onClose={()=>{setV("");setErr("");onClose();}} title="🔐 Admin PIN Required">
      <p style={{fontSize:13,color:C.gray600,marginBottom:16}}>Enter the Admin PIN to continue.</p>
      <Fld label="PIN" req err={err}><Inp type="password" value={v} onChange={e=>setV(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} autoFocus/></Fld>
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial?"✏️ Edit Contact":"🙋 Add New Contact"}>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:"10px 14px"}}>
        ...
        <Fld label={L.whatsapp||"WhatsApp No."} err={errs.wa} col="2"><Inp value={f.wa} onChange={set("wa")} maxLength={10} error={errs.wa} placeholder="optional"/></Fld>
        <Fld label={L.mandal||"Mandal"} req err={errs.mandal} col="1">
          <Sel value={f.mandal} onChange={e=>setF(p=>({...p,mandal:e.target.value,panchayat:""}))} error={errs.mandal}>
            <option value="">{`— Select ${L.mandal||"Mandal"} —`}</option>
            {settings.mandals.map(m=><option key={m.name}>{m.name}</option>)}
          </Sel>
        </Fld>
        <Fld label={L.panchayat||"Panchayat"} req err={errs.panchayat} col="2">
          <Sel value={f.panchayat} onChange={set("panchayat")} error={errs.panchayat} disabled={!f.mandal}>
            <option value="">{`— Select ${L.panchayat||"Panchayat"} —`}</option>
            {panchs.map(p=><option key={p}>{p}</option>)}
          </Sel>
        </Fld>
        <Fld label={L.village||"Village"} col="1"><Inp value={f.village} onChange={set("village")} placeholder="optional"/></Fld>
        <Fld label={L.caste||"Caste"} req err={errs.caste} col="2">
        ...
        <Fld label={`${L.booth||"Booth"} No.`} err={errs.bno} col="2"><Inp value={f.bno} onChange={e=>setF(p=>({...p,bno:e.target.value.replace(/\D/g,"")}))} error={errs.bno} placeholder="numeric"/></Fld>
        <Fld label={L.boothName||"Booth Name"}><Inp value={f.bnm} onChange={set("bnm")} placeholder="optional"/></Fld>
        <Fld label={L.tag||"Tag"} req err={errs.tag}>
...
function BoothForm({open,onClose,initial,settings,onSave,existingBooths,saving}) {
  ...
  const submit=()=>{if(!validate())return;onSave({...f,voters:parseInt(f.voters)||0,elec:f.elec.map(e=>({cast:parseInt(e.cast)||0,votes:e.votes.map(v=>parseInt(v)||0)}))});};
  ...
  return (
    <Modal open={open} onClose={onClose} title={initial?`✏️ Edit ${L.booth||"Booth"}`:`📍 Add ${L.booth||"Booth"}`} wide>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"10px 14px"}}>
        <Fld label={`${L.booth||"Booth"} No.`} req err={errs.bno} col="1"><Inp value={f.bno} onChange={e=>setF(p=>({...p,bno:e.target.value.replace(/\D/g,"")}))} error={errs.bno} placeholder="numeric" booth/></Fld>
        <Fld label={L.boothName||"Booth Name"} col="2"><Inp value={f.bnm} onChange={e=>setF(p=>({...p,bnm:e.target.value}))} placeholder="optional" booth/></Fld>
        <Fld label="Rating" col="3"><Sel value={f.rating} onChange={e=>setF(p=>({...p,rating:e.target.value}))} booth><option value="">— Optional —</option><option value="A">A — Generally Wins</option><option value="B">B — Mediocre</option><option value="C">C — Generally Loses</option></Sel></Fld>
        <Fld label={L.mandal||"Mandal"} req err={errs.mandal} col="1"><Sel value={f.mandal} onChange={e=>setF(p=>({...p,mandal:e.target.value,panchayat:""}))} error={errs.mandal} booth><option value="">— Select —</option>{settings.mandals.map(m=><option key={m.name}>{m.name}</option>)}</Sel></Fld>
        <Fld label={L.panchayat||"Panchayat"} req err={errs.panchayat} col="2"><Sel value={f.panchayat} onChange={e=>setF(p=>({...p,panchayat:e.target.value}))} error={errs.panchayat} disabled={!f.mandal} booth><option value="">— Select —</option>{panchs.map(p=><option key={p}>{p}</option>)}</Sel></Fld>
        ...
        {[0,1,2].map(i=>(<Fld key={i} label={`Top ${L.caste||"Caste"} ${i+1}`} col={String(i+1)}><Sel value={f.castes[i]} onChange={e=>{const c=[...f.castes];c[i]=e.target.value;setF(p=>({...p,castes:c}));}} booth><option value="">— Select —</option>{settings.castes.map(c=><option key={c}>{c}</option>)}</Sel></Fld>))}
      </div>
      <div className="election-history-section" style={{marginBottom:12}}>
      ...
      {tab==="geo"&&(<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16}}>
        <div>
          ...
          <div style={{border:`1.5px solid ${C.gray200}`,borderRadius:10,overflow:"hidden",maxHeight:180,overflowY:"auto",marginBottom:8}}>
            ...
            {s.mandals.length===0&&<div style={{padding:12,color:C.gray400,fontSize:12}}>No mandals</div>}
          </div>
          <div style={{display:"flex",gap:6}}><Inp value={nm} onChange={e=>setNm(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addM()} placeholder="New mandal…"/><Btn v="primary" onClick={addM} style={{padding:"9px 14px"}}>+</Btn></div>
        </div>
        <div>
      ...
      {tab==="labels"&&(
        <div>
          <div style={{background:C.primaryLight,borderRadius:10,padding:"10px 14px",marginBottom:16,fontSize:12,color:C.primary,lineHeight:1.7}}>
            <b>Customize field names for your state.</b><br/>e.g. Rename "Mandal" → "Tehsil", "Panchayat" → "Gram Sabha"
          </div>
  ...
  return (
    <Modal open={open} onClose={onClose} title="📊 Import Booths from Excel/CSV">
      <div style={{background:C.tealLight,borderRadius:10,padding:"12px 14px",marginBottom:14,fontSize:12,color:C.boothDark,lineHeight:1.8,fontWeight:500}}>
        <b>CSV columns:</b> BoothNo, BoothName, Panchayat, Mandal, TotalVoters<br/>
        Existing booth numbers → updated | New ones → added
      </div>
      <Fld label="Paste CSV"><textarea value={txt} onChange={e=>setTxt(e.target.value)} style={{width:"100%",padding:"9px 12px",fontSize:12,fontFamily:"monospace",border:`1.5px solid ${C.teal}66`,borderRadius:9,background:"#FAFAFA",minHeight:80,resize:"vertical",outline:"none"}} placeholder="42,Primary School Gaighat,Gaighat,Patna Sadar,1240"/></Fld>
      <input ref={ref} type="file" accept=".csv" style={{marginBottom:12,fontSize:12}} onChange={e=>{const file=e.target.files[0];if(!file)return;const r=new FileReader();r.onload=ev=>setTxt(ev.target.result);r.readAsText(file);}}/>
      <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
        <Btn v="ghost" onClick={onClose}>Cancel</Btn>
...
const COLS=["Timestamp","Name","Phone","WhatsApp","Mandal","Panchayat","Village","BoothNo","BoothName","Tag","Caste","Gender","Notes"];
function doGet(){const sheet=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET);const[header,...rows]=sheet.getDataRange().getValues();const data=rows.map(r=>Object.fromEntries(header.map((h,i)=>[h,r[i]])));return ContentService.createTextOutput(JSON.stringify({status:"ok",data})).setMimeType(ContentService.MimeType.JSON);}
function doPost(e){try{const sheet=SpreadsheetApp.getActiveSpreadsheet().getSheetByName(SHEET);if(sheet.getLastRow()===0)sheet.appendRow(COLS);const b=JSON.parse(e.postData.contents);if(b.action==="add"){sheet.appendRow([new Date().toISOString(),b.name||"",b.phone||"",b.wa||"",b.mandal||"",b.panchayat||"",b.village||"",b.bno||"",b.bnm||"",b.tag||"",b.caste||"",b.gender||"",b.notes||""]);return ContentService.createTextOutput(JSON.stringify({status:"ok"})).setMimeType(ContentService.MimeType.JSON);}}catch(err){return ContentService.createTextOutput(JSON.stringify({status:"error",message:err.toString()})).setMimeType(ContentService.MimeType.JSON);}}`;

function SheetsModal({open,onClose,settings,setSettings}) {
  ...
  return (
    <Modal open={open} onClose={onClose} title="🔗 Google Sheets Integration" wide>
      <div style={{background:C.primaryLight,borderRadius:10,padding:"12px 14px",marginBottom:14,fontSize:12,color:C.primary,lineHeight:1.8,fontWeight:500}}>
        <b>One-time setup:</b><br/>1. Google Sheets → create sheet named <b>Contacts</b><br/>2. Extensions → Apps Script → paste code → Deploy → Web App<br/>3. Execute as: Me | Access: Anyone → copy URL → paste below
      </div>
      <div style={{marginBottom:12}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><span style={{fontSize:12,fontWeight:700}}>Apps Script code</span><Btn v="ghost" onClick={()=>navigator.clipboard.writeText(APPS_SCRIPT).then(()=>alert("Copied!"))} style={{padding:"5px 12px",fontSize:11}}>📋 Copy</Btn></div>
        <pre style={{background:C.gray50,border:`1px solid ${C.gray200}`,borderRadius:8,padding:10,fontSize:9,maxHeight:90,overflow:"auto",whiteSpace:"pre-wrap"}}>{APPS_SCRIPT}</pre>
      </div>
      <Fld label="Web App URL"><Inp value={url} onChange={e=>setUrl(e.target.value)} placeholder="https://script.google.com/macros/s/…/exec"/></Fld>
      <div style={{display:"flex",gap:10,justifyContent:"flex-end"}}>
        <Btn v="ghost" onClick={onClose}>Cancel</Btn>
        <Btn v="primary" onClick={()=>{setSettings(s=>({...s,sheetsUrl:url}));onClose();}}>Save</Btn>
      </div>
    </Modal>
  );
}
...
function ContactDetail({contact,settings,onEdit,onDelete}) {
  ...
  return(<>
    ...
    <div style={{padding:"14px 16px",flex:1,overflowY:"auto"}}>
      {[
        [`📞 Phone`,contact.phone],
        [`💬 ${L.whatsapp||"WhatsApp"}`,contact.wa],
        [`🏷️ ${L.caste||"Caste"}`,contact.caste],
        [`👤 Gender`,contact.gender],
        [`🏘️ ${L.village||"Village"}`,contact.village],
        [`🗺️ ${L.mandal||"Mandal"}`,contact.mandal],
        [`🏛️ ${L.panchayat||"Panchayat"}`,contact.panchayat],
...
export default function App() {
  ...
  const [showPinAdmin, setShowPinAdmin] = useState(false); // for in-app pin actions
  ...
  // ── In-app pin admin (for export etc) ────────────────────────────────────
  const [pinIsAdmin, setPinIsAdmin] = useState(false);
  const reqPinAdmin = useCallback(action=>{
    if(isAdmin||pinIsAdmin){action();return;}
    setOtpAction(()=>action); setShowOTP(true);
  },[isAdmin,pinIsAdmin]);
  ...
  const syncToSheets=async contact=>{
    ...
    try{await fetch(settings.sheetsUrl,{method:"POST",body:JSON.stringify({action:"add",...contact}),headers:{"Content-Type":"text/plain"}});setSyncStatus("ok");setTimeout(()=>setSyncStatus(""),3000);}
  ...
  const handleSaveConst=async f=>{
    setSaving(true);
    try{const updated={...settings,...f};await saveSettings(updated,workspace.id);setSettingsState(updated);setShowConst(false);showToast("Saved ✓");}
    catch(err){showToast("Error: "+err.message,"error");}
    setSaving(false);
  };
  ...
  const handleImport=async txt=>{
    ...
    const rows=lines.slice(start).map(ln=>{const c=ln.split(",").map(v=>v.replace(/^"|"$/g,"").trim());return{name:c[0],phone:c[1]||"",wa:c[2]||"",mandal:c[3]||"",panchayat:c[4]||"",village:c[5]||"",bno:c[6]||"",bnm:c[7]||"",tag:TAGS_SET.has(c[8])?c[8]:"",caste:c[9]||"",gender:c[10]||"",notes:c[11]||""};}).filter(r=>r.name);
    setSaving(true);
    try{
      const created=await Promise.all(rows.map(r=>insertContact(r,workspace.id)));
  ...
  const handleBoothExcel=async rows=>{
    setSaving(true);
    try{
      const results=await Promise.all(rows.map(r=>upsertBoothByBno({bno:r.bno,bnm:r.bnm||"",mandal:r.mandal||"",panchayat:r.panchayat||"",voters:parseInt(r.voters)||0,rating:"",castes:["","",""],elec:[{cast:0,votes:[0,0,0]},{cast:0,votes:[0,0,0]},{cast:0,votes:[0,0,0]}],notes:""},workspace.id)));
      const fresh=await fetchBooths(workspace.id); setBooths(fresh);
  ...
  const exportCSV=()=>{
    const rows=[["Name","Phone","WhatsApp","Mandal","Panchayat","Village","BoothNo","BoothName","Tag","Caste","Gender","Notes"]];
    filteredC.forEach(c=>rows.push([c.name,c.phone,c.wa,c.mandal,c.panchayat,c.village,c.bno,c.bnm,c.tag,c.caste,c.gender,c.notes]));
  ...
  return (
    <div id="app-root" style={{display:"flex",flexDirection:"column",height:"100vh",background:C.white,fontFamily:"-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,sans-serif",fontSize:13,overflow:"hidden"}}>
      ...
      <div id="const-bar" style={{display:"flex",alignItems:"center",background:"linear-gradient(135deg,#1E1B4B,#312E81)",flexShrink:0,width:"100%",minHeight:42,maxHeight:42,overflowX:"auto",overflowY:"hidden",position:"relative",zIndex:100}}>
        ...
        <div style={{marginLeft:"auto",display:"flex",gap:6,paddingRight:12,flexShrink:0}}>
          {settings.sheetsUrl&&<button onClick={()=>{}} style={{padding:"5px 10px",fontSize:10,background:"transparent",border:"1px solid #4338CA",borderRadius:6,color:"#C7D2FE",cursor:"pointer",fontWeight:600,fontFamily:"inherit",whiteSpace:"nowrap"}}>⬇️ Pull</button>}
          <button onClick={()=>setShowConst(true)} style={{padding:"5px 10px",fontSize:10,background:"transparent",border:"1px solid #4338CA",borderRadius:6,color:"#C7D2FE",cursor:"pointer",fontWeight:600,fontFamily:"inherit",whiteSpace:"nowrap"}}>✏️ Edit</button>
        </div>
      </div>

      <div id="app-body" style={{display:"flex",flex:1,overflow:"hidden",minHeight:0}}>
          ...
          {screen==="booths"&&<>
            <div id="booth-hero" style={{background:`linear-gradient(135deg,${C.tealLight},#ECFDF5)`,padding:"12px 16px",borderBottom:`1px solid ${C.teal}33`,flexShrink:0}}>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                <select value={bfR} onChange={e=>setBfR(e.target.value)} style={{flex:1,minWidth:100,padding:"8px 10px",fontSize:12,border:`1.5px solid ${C.teal}55`,borderRadius:9,background:bfR?C.tealLight:C.white,color:bfR?C.teal:C.gray600,fontWeight:bfR?700:400,outline:"none"}}>
                  <option value="">All Ratings</option><option>A</option><option>B</option><option>C</option>
                </select>
                <select value={bfP} onChange={e=>setBfP(e.target.value)} style={{flex:2,minWidth:140,padding:"8px 10px",fontSize:12,border:`1.5px solid ${C.teal}55`,borderRadius:9,background:bfP?C.tealLight:C.white,color:bfP?C.teal:C.gray600,fontWeight:bfP?700:400,outline:"none"}}>
                  <option value="">All {L.panchayat||"Panchayat"}s</option>{allPanchs.map(p=><option key={p}>{p}</option>)}
                </select>
              </div>
            </div>
      ...
      {/* MOBILE DETAIL */}
      {showMobileDetail&&(
        <div onClick={e=>e.target===e.currentTarget&&setShowMobileDetail(false)} style={{position:"fixed",inset:0,background:"rgba(17,24,39,.6)",display:"flex",alignItems:"flex-end",justifyContent:"center",zIndex:800,backdropFilter:"blur(3px)"}}>
          <div style={{background:C.white,borderRadius:"20px 20px 0 0",width:"100%",maxHeight:"85vh",overflowY:"auto",paddingBottom:80,transition:"transform 0.3s ease"}}
            onTouchStart={e=>{e.currentTarget._touchStartY=e.touches[0].clientY;e.currentTarget._touchStartTime=Date.now();}}
            onTouchMove={e=>{const dy=e.touches[0].clientY-e.currentTarget._touchStartY;if(dy>0)e.currentTarget.style.transform=`translateY(${dy}px)`;}}
            onTouchEnd={e=>{const dy=e.changedTouches[0].clientY-e.currentTarget._touchStartY;const dt=Date.now()-e.currentTarget._touchStartTime;if(dy>100||dy/dt>0.5)setShowMobileDetail(false);else e.currentTarget.style.transform="translateY(0)";}}>
            <div style={{width:40,height:5,background:C.gray200,borderRadius:3,margin:"12px auto 4px"}}/>
            <div style={{textAlign:"center",fontSize:10,color:C.gray400,marginBottom:8,fontWeight:500}}>Swipe down to close</div>
            {screen==="contacts"
              ?<ContactDetail contact={contacts.find(c=>c.id===selC?.id)||null} settings={settings} onEdit={c=>{setEditC(c);setShowAdd(true);setShowMobileDetail(false);}} onDelete={id=>{handleDeleteContact(id);setShowMobileDetail(false);}}/>
              :<BoothDetail booth={booths.find(b=>b.id===selB?.id)||null} settings={settings} onEdit={b=>{setEditB(b);setShowBAdd(true);setShowMobileDetail(false);}} onDelete={id=>{handleDeleteBooth(id);setShowMobileDetail(false);}}/>
            }
          </div>
        </div>
      )}
